...
# ==================== 設定エリア ====================
# Discordトークンは環境変数（Render側で設定）から安全に読み込みます
...
ROUTES = {
    ...
    "青梅線": {
        ...
        "stations": {
            ...
            "宮ノ平": {
                "station_id": "2241820130",
                "lat": 35.7930,
                "lon": 139.2380,
            },
            "日向和田": {
                "station_id": "2241820140",
                "lat": 35.7950,
                "lon": 139.2250,
            },
            "石神前": {
                "station_id": "2241820150",
                "lat": 35.7960,
                "lon": 139.2150,
            },
            "二俣尾": {
                "station_id": "2241820160",
                "lat": 35.8020,
                "lon": 139.2050,
            },
...
headers = {
    ...
    "user-agent": (
        "Mozilla/5.0 (Linux; Android 14; Pixel 9 Pro Fold) AppleWebKit/537.36"
...
async def handle_index(request):
  ...
  for r_name, r_data in ROUTES.items():
    ...
    route_tabs += (
        f'<button class="route-btn {active_class}"'
        f" onclick=\"switchRoute('{r_name}')\">{r_name}</button>"
    )

    display_style = "block" if r_name == first_route else "none"
    station_buttons_container += (
        f'<div id="stations-{r_name}" class="station-grid-group"'
        f' style="display: {display_style};">'
    )
    for s_name in r_data["stations"].keys():
      station_buttons_container += (
          f'<button class="station-btn"'
          f" onclick=\"setStation('{r_name}', '{s_name}')\">{s_name}</button>"
      )
  ...
  html = (
    ...
    <head>
        ...
        <style>
            ...
            .station-grid-group { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; max-width: 400px; margin: 0 auto; }
            .station-btn { background: #2f3640; color: white; border: none; padding: 12px; font-size: 14px; border-radius: 8px; cursor: pointer; }
    ...
    <body>
        ...
        <script>
            function switchRoute(routeName) {
                document.querySelectorAll('.station-grid-group').forEach(el => el.style.display = 'none');
                document.getElementById('stations-' + routeName).style.display = 'grid';
  ...
  try:
    ...
    if req_type == "gps":
      ...
      if closest_station:
        ...
        print(
            f"📍 GPS同期完了 → [{current_line_name}]"
            f" {current_station_name}駅に変更しました！"
        )
      ...
      if r_name in ROUTES and s_name in ROUTES[r_name]["stations"]:
        ...
        print(
            f"👆 手動切替 → [{current_line_name}]"
            f" {current_station_name}駅に変更しました！"
        )

    return web.json_response({
        "status": "success",
        "route": current_line_name,
...
async def play_audio_in_vc(audio_filename):
  try:
    ...
    voice_client = discord.utils.get(
        bot.voice_clients, guild=target_channel.guild
    )
      ...
      try:
        ...
        if response.status_code == 200:
          ...
          for train in train_list:
            ...
            end_station = train.get("END_STATION")
            ...
            if unit_info == "5" and std_tm:
              ...
              if 0 <= remaining_seconds <= TARGET_SECONDS_BEFORE:
                ...
                print(
                    f"🎯 【{direction.upper()}線 直前検知"
                    f" @{current_line_name}/{current_station_name}】 列車:"
                    f" {train_id} ({train_name}) | 残り約 {remaining_seconds}秒"
                )

                if current_time - last_played[direction] >= dynamic_cooldown:
                  audio_file = (
                      SOUND_NOBORI if direction == "up" else SOUND_KUDARI
                  )
